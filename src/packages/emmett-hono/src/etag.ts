...
export const getETagFromIfMatch = (request: HonoRequest): ETag => {
  ...
  // Hono's header() returns a single string or undefined, not an array
  return etag as ETag;
};

export const getETagFromIfNotMatch = (request: HonoRequest): ETag => {
  const etag = request.header(HeaderNames.IF_NOT_MATCH);

  if (etag === undefined) {
    throw new Error(ETagErrors.MISSING_IF_NOT_MATCH_HEADER);
  }

  // Hono's header() returns a single string or undefined
  return etag as ETag;
};

export const setETag = (c: Context, etag: ETag): void => {
  c.header(HeaderNames.ETag, etag as string);
};

export const getETagValueFromIfMatch = (request: HonoRequest): string => {
